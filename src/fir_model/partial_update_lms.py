#!/usr/bin/env python3
"""
partial_update_lms.py

Partial-Update LMS (M-Max) adaptive FIR identification.

Workflow:
  1. Load frequency response from a CSV file (omega, ReG, ImG).
  2. Interpolate to a uniform grid via PCHIP and compute the impulse
     response using Hermitian-symmetric IFFT.
  3. Trim by cumulative energy and apply a Hanning window.
  4. Load recorded input/output from a MAT file, resample if needed.
  5. Run partial-update (M-Max) LMS: at each step, only the M_UPDATE
     taps with the largest gradient magnitude are updated.
  6. Report RMSE, NRMSE, and R2 error metrics and save results.

Originally fir/reduce_cost.py. All Japanese comments translated to
English; computational logic unchanged.

Constants (tunables):
    FRF_CSV        : CSV with columns [omega, ReG, ImG].
    IO_FILE        : MAT with recorded I/O data ([time; y; u]).
    LAMBDA_FACTOR  : (reserved, not used in LMS -- kept for API parity).
    ENERGY_CUT     : Cumulative energy threshold for tap selection.
    PLOT_RATE      : Samples between live-plot refreshes.
    MU             : LMS step size.
    M_UPDATE       : Number of taps updated per sample (M-Max strategy).
"""
...
# Tunables
# ------------------------------------------------------------------ #
FRF_CSV = './fir/data/predicted_G_values.csv'  # FRF CSV: [omega, ReG, ImG]
IO_FILE = './fir/data/data_hour.mat'           # recorded I/O to replay
LAMBDA_FACTOR = 0.995      # (reserved, not used in LMS)
ENERGY_CUT = 0.99          # keep >= 99% of |g| energy
PLOT_RATE = 100            # samples between plot refreshes
MU = 1e-3                  # LMS step size
M_UPDATE = 10              # taps updated per sample (M-Max partial update)


def run_partial_update_lms(
    frf_csv: str = FRF_CSV,
    io_file: str = IO_FILE,
    energy_cut: float = ENERGY_CUT,
    plot_rate: int = PLOT_RATE,
    mu: float = MU,
    m_update: int = M_UPDATE,
):
    """
    Run partial-update (M-Max) LMS identification.

    Args:
        frf_csv:    Path to FRF CSV (omega, ReG, ImG).
        ...
        mu:         LMS step size.
        m_update:   Number of taps to update per sample.
    ...
    var_names = [name for name in mat_data.keys()
    ...
    yhat = np.zeros(N)
    err = np.zeros(N)
    H = np.zeros((L, N))  # Coefficient history
    ...
    for n in range(N):
        ...
        # Partial-update LMS: update only the M taps with largest |delta|
        delta = mu * phi * err[n]
