#!/usr/bin/env python3
"""
kernel_regularized.py

Kernel-regularized FIR identification (time-domain) with DC, SS(2),
and SI kernels.

Predict the current output from input/output sequences by estimating a
finite-length impulse response g via Tikhonov/GP regularization.
Hyperparameters are selected by maximizing the GP marginal likelihood
using low-rank identities (no N x N matrices).
...
Usage (CLI):
  python kernel_regularized.py --io ./fir/data/data_hour.mat --kernel dc --out ./test_output

Notes:
- Indices use causal convention with g[0] multiplying u[n]. This is a
  0-lag FIR form common in signal processing. The guide's tau>=1 form
  corresponds to g shifted by 1.

Originally pure_fir_model.py. All Japanese comments translated to
...
def _load_io_mat(
    io_mat: Path,
    nmax: Optional[int] = 100_000,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load (t, u, y) from a MAT file, optionally truncating to nmax samples."""
    ...
    if mat.ndim != 2 or (mat.shape[0] != 3 and mat.shape[1] != 3):
        raise ValueError(
            "Expected a 3xN or Nx3 matrix with rows [time; y; u]")
    if mat.shape[0] == 3:
        t_all = mat[0, :].ravel()
        y_all = mat[1, :].ravel()
        u_all = mat[2, :].ravel()
...
def _kernel_dc(
    L: int, c: float, lam: float, rho: float,
) -> np.ndarray:
    """Diagonal/Correlated kernel:
    k(i,j) = c * lam^{(i+j)/2} * rho^{|i-j|}, i,j = 0..L-1.
    """
    i = np.arange(L)[:, None]
    j = np.arange(L)[None, :]
...
def _kernel_ss2(L: int, c: float, lam: float) -> np.ndarray:
    """Second-order stable spline: K = c * (C H)(C H)^T.
    H: lower-triangular with H[i,j] = lam^{i-j}.
    C: cumulative sum matrix (lower-triangular ones).
    """
    idx = np.arange(L)
...
def _kernel_si(
    L: int, r: float, omega: float, q: float, barlam: float,
) -> np.ndarray:
...
def _build_kernel(
    L: int, kernel: str, theta: np.ndarray,
) -> np.ndarray:
...
# ------------------------------------------------------------------ #
# Marginal likelihood (low-rank identity)
# ------------------------------------------------------------------ #
...
def _nll(
    ...
    try:
        Ls = cholesky(Ssym + jitter * np.eye(L), lower=True,
                      check_finite=False)
    except np.linalg.LinAlgError:
        Ls = cholesky(Ssym + 1e-6 * np.eye(L), lower=True,
                      check_finite=False)
...
def _posterior_mean_g(
    ...
    Ssym = np.eye(L) + (Lk.T @ (G @ Lk)) / max(sig2, 1e-30)
    Ls = cholesky(Ssym + 1e-10 * np.eye(L), lower=True,
                  check_finite=False)
    w = Lk.T @ b
    x = cho_solve((Ls, True), w, check_finite=False)
...
def run_fir_experiment(cfg: FIRConfig) -> Dict[str, object]:
    ...
    if Path(cfg.io_mat).exists():
        ...
        y_raw = (y_clean + 0.02 * np.std(y_clean)
                 * rng.standard_normal(N)).astype(float)
    N = len(u_raw)
    if cfg.demean:
    ...
    if kname == "dc":
        theta0 = np.array([math.log(1.0),
                           math.log(0.98 / (1 - 0.98)),
                           np.arctanh(0.8)])
    elif kname == "ss":
        theta0 = np.array([math.log(1.0),
                           math.log(0.96 / (1 - 0.96))])
    elif kname == "si":
        theta0 = np.array([math.log(0.98 / (1 - 0.98)),
                           math.log(0.25),
                           math.log(1e-2),
                           math.log(0.96 / (1 - 0.96))])
    else:
        raise ValueError("Unknown kernel")
    # Append noise parameter
    ...
    # Posterior mean g
    g_hat, sig2_hat, K_hat = _posterior_mean_g(
        theta_hat, G, b, cfg.L, cfg.kernel)
    ...
    nrmse = float(1.0 - np.linalg.norm(e_tail)
                  / (np.linalg.norm(y_tail - np.mean(y_tail)) + 1e-12))
    r2 = float(1.0 - np.sum(e_tail ** 2)
               / (np.sum((y_tail - np.mean(y_tail)) ** 2) + 1e-12))
    fit = float(100.0 * (1.0 - np.linalg.norm(e_tail)
                / (np.linalg.norm(y_tail - np.mean(y_tail)) + 1e-12)))
    ...
    ax.stem(np.arange(cfg.L), g_hat, linefmt="C0-", markerfmt="C0o",
            basefmt="k-")
    ...
    # Save artifacts
    np.savez(cfg.out_dir / "fir_coefficients.npz",
             g_hat=g_hat, K=K_hat, kernel=cfg.kernel, L=cfg.L)
    metrics = {
        "rmse": rmse,
        "nrmse": nrmse,
        "r2": r2,
    ...
    with open(cfg.out_dir / "fir_metrics.json", "w",
              encoding="utf-8") as f:
...
def _parse_argv() -> Optional[FIRConfig]:
    ...
    p = argparse.ArgumentParser(
        description="Kernel-regularized FIR identification")
    p.add_argument("--io", type=str,
                   default="./fir/data/data_hour.mat",
                   help="MAT file with [time;y;u]")
    p.add_argument("--out", type=str, default="./test_output_test",
                   help="Output directory")
    p.add_argument("--kernel", type=str, default="dc",
                   choices=["dc", "ss", "si"], help="Kernel type")
    p.add_argument("--L", type=int, default=128,
                   help="FIR length (memory)")
    p.add_argument("--starts", type=int, default=3,
                   help="Number of multi-starts for ML")
