...
def run_test():
    ...
    fir_eval_found = (
        "FIR evaluation" in result2.stdout or
        "Creating FIR evaluation" in result2.stdout
    )
    print(f"Found FIR evaluation messages: {fir_eval_found}")

    # Check for validation data loading
    validation_loaded = (
        "Loading validation data" in result2.stdout or
        "Validation data loaded" in result2.stdout
    )
